...
export class AssignNewUserCommandHandler implements ICommandHandler<AssignNewUserCommand> {
  ...
  async execute(command: AssignNewUserCommand) {
    ...
    if (!space) throw new InvalidArgumentException(`Space with ${command.spaceId} doesn't exist.`);
    if (space.state.admin !== command.applierId) {
      throw new InvalidArgumentException('User doesn\'t have authority to assign new users.');
    }
